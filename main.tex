...
Figure \ref{fig:ensemble_panel} \todo[inline]{move the lat, lon to the caption right now they're confusing, parenthases help (43.37, -123.25), either remove titles and just have (a), (b) etc and put info in caption or do b) DLV3P c) PAN in titles and in the caption (b)-(d) are single models (e) shows the ensemble of DLV3P, PAN etc} shows an example of smoke plume detection from the testing dataset. The ensemble predictions show smoother boundaries, making the prediction more comparable to the human-drawn polygon annotations.
...
We explore two schemes for building ensembles of deep learning models that both improve on testing set IoU and make the predictions more realistic\todo{dont use 'realistic'} than those of a single model. However, further investigation is required to understand why the architecture-based ensemble decreases in performance after 8 models, and how a combination of the two ensemble schemes\todo{you want to ensemble the ensembles? might be confusing} may perform. Additionally, we are also experimenting with regionally-trained models\todo{add info on why this is a promising direction? find lit on geographical variations in model performance (smokeviz mentions a paper on that \url{https://horizon.documentation.ird.fr/exl-doc/pleins_textes/pleins_textes_7/b_fdi_51-52/010016029.pdf)}}, to further improve smoke detection. In the future, the application of these ensemble techniques are expected to aid in fire and hazard management by automating the monitoring of smoke in real-time from satellite imagery. This tool can be used to provide more frequent and consistent detection of smoke plumes, ultimately supporting climate resilience and adaptation strategies.
